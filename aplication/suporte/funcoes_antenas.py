...
def gera_regras(antenas, agp, geovis, reg_ant, reg_alvo, indicador, geofiltro, faixa_sup, conf_base, indicador_base):
    ...
    if agp == 'disp':
        n = antenas['dispositivos'].sum()
    else:
        n = antenas['conexoes'].sum()
    ...
    if agp == 'disp':
        sup_cons = int(df_cons['dispositivos'].sum() / n * 10000) / 100
        qt_cons = int(df_cons['dispositivos'].sum())
    else:
        sup_cons = int(df_cons['conexoes'].sum() / n * 10000) / 100
        qt_cons = int(df_cons['conexoes'].sum())
    ...
    agrupador = "cod" + geovis
    nomegeovis = geovis

    # Cria valor de referência para o suporte
    if agp == 'disp':
        df = df.groupby(by=[df[agrupador]]).agg(sup_alvo=('dispositivos', 'sum')).reset_index()
        df_ant = df_ant.groupby(by=[df_ant[agrupador]]).agg(sup_ant=('dispositivos', 'sum')).reset_index()
        df = df_ant.merge(df, on=[agrupador], how='left', suffixes=['1', '2'], indicator=False, sort=True)
        df = df.fillna(0)
        df['sup_alvo'] = df['sup_alvo'].astype(int)
    else:
        df = df.groupby(by=[df[agrupador]]).agg(sup_alvo=('conexoes', 'sum')).reset_index()
        df_ant = df_ant.groupby(by=[df_ant[agrupador]]).agg(sup_ant=('conexoes', 'sum')).reset_index()
        df = df_ant.merge(df, on=[agrupador], how='left', suffixes=['1', '2'], indicator=False, sort=True)
        df = df.fillna(0)
        df['sup_alvo'] = df['sup_alvo'].astype(int)
    ...
    if len(geofiltro) == 0:
        localidade = localidade[[agrupador, nomegeovis, 'lat', 'long']]
        ...
        localidade = localidade[[agrupador, nomegeovis, 'lat', 'long', geofiltro[0]]]
        df = df.merge(localidade, on=[agrupador], how='inner', suffixes=['1', '2'], indicator=False, sort=True)
    ...
    if indicador == 'sup':
        ...
        #df['faixaindicador'] = df.apply(lambda x: 'from ' + str(int((0 + (int(x['clasvis']) - 1) * (indmax - 0 + 0.000499) / classes) * 100) / 100) + ' to ' + str(int((0 + int(x['clasvis']) * (indmax - 0 + 0.000599) / classes) * 100) / 100) + '%', axis=1)
    ...
    df['txtsupant'] = df.apply(lambda x: str(x['supant']) + '% => Qt Ant/Tot' + ' '+agp.title()+' = ' + str(x['qtd_ant']) + '/' + str(n), axis=1)
    df['txtsup'] = df.apply(lambda x: str(x['sup']) + '% => Qt Alvo/Tot' + ' '+agp.title()+' = ' + str(x['qtd_alvo']) + '/' + str(n), axis=1)
    ...
    #Executa o geofiltro
    if len(geofiltro) > 0:
        df = df.loc[(df[geofiltro[0]] == str(geofiltro[1]))]
...
def configura_mapa(dfgeovis, antenas, agp, geovis, reg_ant, reg_alvo, indicador, geofiltro, base):

    # Define o caminho dos arquivos de coordenadas geográficas
    path = os.path.join(os.sep, 'Users', 'ANDERSON', 'PycharmProjects', 'mapa_de_calor', 'covid', 'aplicativo', 'suporte', '')

    # Gera a base de regras para a geovisualização dos dados
    #dfgeovis = gera_regras(antenas, agp, geovis, reg_ant, reg_alvo, indicador, geofiltro)
    ...
    base = base.rename(columns={'codra_x': 'codra'})
    ...
    listageo = [['codra', 'ra', 12]]

    # Renderizando o mapa com algumas personalizações:
    if len(geofiltro) == 0:
        mapa = folium.Map(location=[-22.9535, -43.4096], zoom_start=10, tiles='OpenStreetMap', name="Título do Mapa")
    else:
        base = base.loc[(base[geofiltro[0]] == geofiltro[1])]
        for geo in listageo:
            if geo[0] == geofiltro[0]:
                coord = pd.read_csv(os.path.join(path, geo[1] + '.csv'), delimiter=";", encoding='latin', usecols=[geo[0], 'lat', 'long'])
                coord = coord.loc[(coord[geo[0]] == geofiltro[1])]
    ...
    Choropleth = folium.Choropleth(
        geo_data=base,
        name="Mapa de Antenas por " + geovis,
        data=dfgeovis,
    ...
    # Gera informações do título(agrupamento)
    if agp == 'disp':
        nmagp = 'Devices'
    else:
        nmagp = 'Conections'
