...
def gera_regras(covid, geovis, reg_ant, reg_alvo, indicador, geofiltro, faixa_sup, conf_base, indicador_base):
    ...
    sup_cons = int(len(df_cons) / len(covid) * 10000) / 100
    ...
    # Cria valor de referência para o suporte
    df = df.groupby(by=[df[agrupador]]).agg(sup_alvo=('codmun', 'size')).reset_index()
    df_ant = df_ant.groupby(by=[df_ant[agrupador]]).agg(sup_ant=('codmun', 'size')).reset_index()
    ...
    # Gera indicadores de referência para cada ano
    anos = [2020, 2021, 2022]
    ...
    for ano in anos:
        ...
        for regra in reg_ant:
            if regra[0] != 'ano':
                ano_ant = ano_ant.loc[(ano_ant[regra[0]] == regra[1])]
        ano_ant = ano_ant.loc[(ano_ant['ano'] == ano)]
        ano_cons = ano_ant.copy()
        for regra in reg_alvo:
            if regra[0] != 'ano':
                ano_cons = ano_cons.loc[(ano_cons[regra[0]] == regra[1])]
        ano_cons = ano_cons.groupby(by=[ano_cons['ano'], ano_cons[agrupador]]).agg(qtd_alvo=('codmun', 'size')).reset_index()
        ano_ant = ano_ant.groupby(by=[ano_ant['ano'], ano_ant[agrupador]]).agg(qtd_ant=('codmun', 'size')).reset_index()
        regano = ano_ant.merge(ano_cons, on=['ano', agrupador], how='left', suffixes=['1', '2'], indicator=False,sort=True)
        regano = regano.fillna(0)
        multidf = pd.concat([regano, multidf], ignore_index=True)
    ...
    if indicador == 'sup':
        #multidf[indicador] = multidf.apply(lambda x: int(x['qtd_alvo'] / n * 10000) / 100, axis=1)
        ...
        multidf[indicador] = multidf.apply(lambda x: int(x['qtd_alvo'] / x['qtd_ant'] * 10000) / 100, axis=1)
        indmax, indmin = multidf[indicador].max(), multidf[indicador].min()
        ...
        if faixa_sup == 0 or indicador_base != indicador:
            ...
            df['faixaindicador'] = df.apply(lambda x: 'from ' + str(int((0 + (int(x['clasvis']) - 1) * ((sup_cons-1) - 0) / 5) * 100) / 100) + ' to ' + str(int((0 + int(x['clasvis']) * (sup_cons-1 - 0) / 5) * 100) / 100) + '%' if x['clasvis'] < 6
                ...
                else 'from ' + str(int((sup_cons-1) * 100) / 100) + ' to ' + str(int((sup_cons+1) * 100) / 100) + '%')), axis=1)
        else:
            ...
            else 'from ' + str(int((conf_base - 1) * 100) / 100) + ' to ' + str(int((conf_base + 1) * 100) / 100) + '%')), axis=1)

    else: # indicador == 'lift':
        multidf[indicador] = multidf.apply(lambda x: int(x['qtd_alvo'] / x['qtd_ant'] / sup_cons * 10000) / 100,axis=1)
        indmax, indmin = multidf[indicador].max(), multidf[indicador].min()
    ...
    df['txtsupant'] = df.apply(lambda x: str(x['supant']) + '% => Qt Ant/Tot' + ' Mun/Ano = ' + str(x['qtd_ant']) + '/' + str(n), axis=1)
    df['txtsup'] = df.apply(lambda x: str(x['sup']) + '% => Qt Alvo/Tot' + ' Mun/Ano = ' + str(x['qtd_alvo']) + '/' + str(n), axis=1)
    df['txtconf'] = df.apply(lambda x: str(x['conf']) + '% => Qt Alvo/Qt Ant = ' + str(x['qtd_alvo']) + '/' + str(x['qtd_ant']), axis=1)
    df['txtlift'] = df.apply(lambda x: str(x['lift']) + ' =>  Conf/Sup'+txtalvo.lower()+' = '+ str(x['conf'])+' / ' + str(sup_cons) + '%', axis=1)
                ...
                if indicador == 'conf':
                    if faixa_sup == 0 or indicador_base != indicador:
                        nova_linha[coluna] = 'from ' + str(int((0 + i * ((sup_cons-1) - 0) / 5) * 100) / 100) + ' to ' + str(int((0 + (i+1) * (sup_cons-1 - 0) / 5) * 100) / 100) + '%' if (i+1) < 6 \
                            ...
                            else 'from ' + str(sup_cons-1) + ' to ' + str(sup_cons+1) + '%'))
                    else:
                        nova_linha[coluna] = 'from ' + str(int((0 + i * ((conf_base-1) - 0) / 5) * 100) / 100) + ' to ' + str(int((0 + (i+1) * (conf_base-1 - 0) / 5) * 100) / 100) + '%' if (i+1) < 6 \
                            ...
                            else 'from ' + str(int((conf_base - 1) * 100) / 100) + ' to ' + str(conf_base + 1) + '%'))
                    ...
                    #nova_linha[coluna] = 'from ' + str(int((0 + i * (indmax - 0 + 0.000499) / classes) * 100) / 100) + ' to ' + str(int((0 + (i+1) * (indmax - 0 + 0.000599) / classes) * 100) / 100) + '%'
...
def preparaBase(filtroBase, df2):
    ...
    df4['fxobpop'] = df4.apply(lambda x: '>= '+str(x['minimo'])+'% a.a.' if x['txobpop'] == 'MUITO ALTA' else '>= '+str(x['minimo'])+' e < '+str(x['maximo'])+'% a.a.', axis=1)
    ...
    df5['fxcasospop'] = df5.apply(lambda x: '>= '+str(x['minimo'])+'% a.a.' if x['txcasospop'] == 'MUITO ALTA' else '>= '+str(x['minimo'])+' e < '+str(x['maximo'])+'% a.a.', axis=1)
...
def configura_mapa(dfgeovis, geovis, reg_ant, reg_alvo, indicador, geofiltro, filtroBase, faixas, base):
    ...
    style_function = ("font-size: 12px; font-weight: bold; color: white;  background-color: grey;")
    ...
    for regra in reg_alvo:
        contagem += 1
        if contagem == 1:
            if regra[0] in ['txobcasos', 'txobpop', 'txcasospop']:
                fx = faixas.loc[(faixas['categoria'] == regra[1]), regra[0]].values[0]
                txtrg = txtrg + '=>' + '(' + regra[0] + '="' + str(regra[1]).lower() + '"(' + fx + '))'
            else:
                txtrg = txtrg + '=>' + '(' + regra[0] + '="' + str(regra[1]).lower() + '")'
        else:
            if regra[0] in ['txobcasos', 'txobpop', 'txcasospop']:
    ...
    # Gera informações do título(período)
    fano = '2020-2022'
    for regra in reg_ant:
        if regra[0] == 'ano':
            fano = str(regra[1])
